...
func _process(_delta):
	#sets the position of the carrot tip to the cursors position when being dragged
	if _dragging:
		var _mousepos := get_viewport().get_mouse_position()
		self.position = Vector2(_mousepos.x, _mousepos.y)


func _on_CarrotTip_input_event(_viewport, _event, _shape_idx):
	if _draggable:
...
func _enableDragging():
	_draggable = true
	print("can drag Carrot Tip now")


func _disableDragging():
	_draggable = false
	print("cant drag Carrot Tip now")
