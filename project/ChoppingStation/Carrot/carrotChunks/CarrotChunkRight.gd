...
func _process(_delta):
	#sets the position of the carrot chunk to the cursors position when being dragged
	if _dragging:
		var _mousepos := get_viewport().get_mouse_position()
		self.position = Vector2(_mousepos.x, _mousepos.y)


func _on_CarrotChunkRight_input_event(_viewport, _event, _shape_idx):
	if _draggable:
		if _event is InputEventMouseButton:
			#enables dragging when carrot tip is held
			if _event.button_index == BUTTON_LEFT and _event.pressed:
				_set_dragging()
			#disables dragging when carrot tip is released
...
func _enableDragging():
	_draggable = true
	print("can drag Carrot Chunk Right now")


func _disableDragging():
	_draggable = false
	print("cant drag Carrot Chunk Right now")
